...
class TestAntlrWebFocusParser(unittest.TestCase):
    ...
    def test_define_file(self):
        variations = [
            ...
            "DEFINE FILE EMPDATA COMPLEX = (SALARY * 1.1) + MAX(BONUS, 1000); END"
        ]
        for code in variations:
            with self.subTest(code=code):
                self.parse(code)
    ...
    def test_compute_commands(self):
        variations = [
            ...
            "TABLE FILE EMPDATA SUM SALARY AND COMPUTE RATIO = SALARY / 1000; END",
            "TABLE FILE EMPDATA SUM SALARY COMPUTE RATIO/D8.2 = SALARY / 1000 AS 'Salary Ratio'; END",
            "TABLE FILE EMPDATA SUM SALARY AND COMPUTE RATIO/D8.2 = SALARY / 1000 AS 'Salary Ratio'; END",
            ...
            "TABLE FILE EMPDATA SUM SALARY COMPUTE MULTI = IF SALARY GT 100000 THEN SALARY * 1.2 ELSE IF SALARY GT 50000 THEN SALARY * 1.1 ELSE SALARY; END"
        ]
        for code in variations:
            with self.subTest(code=code):
                self.parse(code)
